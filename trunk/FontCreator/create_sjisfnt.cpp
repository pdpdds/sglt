...
int mapunicodetoChunk(uint16 unicode) {
...
void TrueTypeFont::renderHangulGlyphs(GlyphList &glyphs, int &count) {
	count = 0;

	for (uint16 fB = 0xac00; fB <= 0xd7af; ++fB) {
...
Glyph::Glyph()
	: fB(0), sB(0), xOffset(0), yOffset(0), height(0), width(0), pitch(0), doublebyte(0),
	  plainData(0) {
}

Glyph::Glyph(const Glyph &r)
	: fB(r.fB), sB(r.sB), xOffset(r.xOffset), yOffset(r.yOffset), doublebyte(r.doublebyte),
...
void Glyph::convertChar8x8(uint8 *dst) const {
	const uint8 *src = plainData;
	dst += yOffset;
